...
p5 <- ggplot(FR, aes(Age, qx, group=Year, color=Year)) + geom_line(aes(color = factor(Year))) +
  geom_vline(xintercept = "105-109", colour = "black", linetype = "dashed") +
   geom_vline(xintercept = "55-59", colour = "black", linetype = "dashed")+
    ylab("Probabilité de décès") + ggtitle("Evolution de la Table de mortalité en France entre 1992 et 2009") +      scale_x_discrete(limits=c("0","1-4","5-9","10-14","15-19","20-24","25-29","30-34","35-39","40-44","45-49","50-54","55-59","60-64","65-69","70-74","75-79","80-84","85-89","90-94","95-99","100-104","105-109","110+"),guide = guide_axis(n.dodge = 2)) +
  theme_classic()
...
FR_reshape_melt <- melt(FR_reshape, id.vars="Age", value.factor = FALSE)
FR_reshape_melt$value <- as.numeric(FR_reshape_melt$value)
FR_reshape_melt <- na.omit(FR_reshape_melt)

p6 <- plotagedensite(FR_reshape_melt,"65-69")
p7 <- plotagedensite(FR_reshape_melt,"75-79")
p8 <- plotagedensite(FR_reshape_melt,"85-89")
p9 <- plotagedensite(FR_reshape_melt,"105-109")

plot_grid(p6,p7,p8,p9)
```
Calcul des ES par tranche d'age.

```{r, echo = FALSE, fig.align = 'center', message=FALSE}
#left_join(DEN_reshape, FR_reshape, by='Age') %>% left_join(EST_reshape, by = 'Age') %>% left_join(HUN_reshape, by = 'Age') %>% left_join(IT_reshape, by = 'Age') %>% left_join(POL_reshape, by = 'Age') %>% left_join(SWE_reshape, by = 'Age') %>% left_join(ENGW_reshape, by = 'Age') %>% left_join(CZE_reshape, by = 'Age')
```
